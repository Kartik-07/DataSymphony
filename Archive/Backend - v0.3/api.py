...
from fastapi import (
    ...
    BackgroundTasks, Body, Path as FastApiPath, status
)
...
# --- App Setup ---
setup_logging() # Ensure logging is set up early
logger = logging.getLogger(__name__)

app = FastAPI(
    title="Enhanced RAG System API (v4.6 - Corrected Depends)", # Updated title
    description="API for querying a RAG system with user authentication, user-specific chat history (including loading/saving in query), SQL routing, and data summarization/ingestion.",
    version="4.6.0", # Updated version
)
...
async def shutdown_event():
    ...
    # Add any specific cleanup here if needed (e.g., closing DB pool)
...
async def get_current_user(token: Annotated[str, Depends(oauth2_scheme)]) -> AuthUserData:
    ...
    try:
        ...
        raise credentials_exception
    except ValueError: # Catch Pydantic validation error for EmailStr
        logger.warning(f"Invalid email format in token's 'sub' field: {email}")
        raise credentials_exception
    except Exception as e:
...
# Keeping request/response specific models here is fine.
...
def run_file_ingestion(temp_path: str, filename: str, indexer: Indexer, summarizer: DataSummarizer, user_email: Optional[str] = None):
    """Background task to summarize and index an uploaded file."""
    log_prefix = f"[Background Task{' for user '+user_email if user_email else ''}]"
    ...
    try:
        ...
        # Ensure temp file is removed even if errors occurred
        try:
...
async def login_for_access_token(
    ...
    try:
        ...
        # You might want a more specific error, but 401 is often used for login failures
        raise HTTPException(
...
# --- FINAL RAG Query Endpoint (Requires Authentication & Handles History) ---
@app.post("/query", response_model=RAGResponse, tags=["RAG Query"], summary="Query RAG Pipeline with History")
...
def process_query_for_user(
    ...
    Loads history if conversation_id is provided, passes it to the pipeline,
    and appends the new interaction back to the specified conversation history.
    """
    user_email = current_user.email
    conversation_id = request.conversation_id
    logger.info(f"Received query from user {user_email}: '{request.query[:100]}...'" + (f" (Conversation ID: {conversation_id})" if conversation_id else " (New Conversation Attempt)"))

    conversation_history: Optional[List[ChatMessage]] = None
    loaded_conv_data: Optional[ConversationData] = None

    # Load history if conversation ID is provided
    if conversation_id:
    ...
    try:
        logger.debug(f"Calling RAG pipeline for user {user_email} query: '{request.query[:50]}...'")
        ...
        logger.error(f"Unexpected error during RAG pipeline query for user {user_email} '{request.query[:100]}...': {e}", exc_info=True)
    ...
    if conversation_id and result.answer and loaded_conv_data:
        ...
        try:
            ...
            user_msg = ChatMessage(sender="user", text=request.query, timestamp=current_time)
    ...
    logger.info(f"Sending RAG response to user {user_email} for query: '{request.query[:100]}...'")
    return result


# --- MODIFIED Ingestion Endpoint (Requires Authentication) ---
@app.post("/ingest_file", response_model=IngestResponse, tags=["Indexing"], status_code=status.HTTP_202_ACCEPTED, summary="Ingest and Index File")
async def ingest_file_endpoint(
    ...
    """Schedules a file for background ingestion, summarization, and indexing."""
    user_email = current_user.email
    original_filename = file.filename if file.filename else "unknown_file"
    log_prefix = f"Ingest request for user {user_email}: {original_filename}"
    logger.info(log_prefix)
    temp_path_str: Optional[str] = None

    try:
        # Define upload directory (consider making this configurable via settings)
        upload_dir = Path(settings.user_history_base_dir / "../uploads_temp") # Example: Place near history
        ...
        try:
             ...
             logger.info(f"Saved temporary file for ingestion: {temp_path_str}")
        except Exception as save_err:
             logger.error(f"Failed to save temporary file {temp_path_str}: {save_err}", exc_info=True)
        ...
        logger.info(f"Scheduled background ingestion for: {original_filename} by user {user_email}")
        # Return success response immediately
        return IngestResponse(
            status="scheduled",
            message=f"Ingestion task for '{original_filename}' scheduled successfully.",
         ...
         if temp_path_str and os.path.exists(temp_path_str):
             ...
             except OSError: logger.error(f"Failed to clean up temp file on HTTPException: {temp_path_str}")
         raise http_exc
    except Exception as e:
        logger.error(f"Failed to schedule ingestion for {original_filename}: {e}", exc_info=True)
        # Clean up temp file on unexpected error during scheduling
        if temp_path_str and os.path.exists(temp_path_str):
            try: os.remove(temp_path_str)
            except OSError: logger.error(f"Failed to clean up temp file on error: {temp_path_str}")
