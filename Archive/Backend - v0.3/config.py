...
try:
    settings.user_history_base_dir.mkdir(parents=True, exist_ok=True)
    logger.info(f"Chat history directory checked/created at: {settings.user_history_base_dir}")
except Exception as e:
    logger.error(f"Failed to create chat history directory '{settings.user_history_base_dir}': {e}", exc_info=True)
